...
int MIAddPostingList(MemIndex index, char *word, DocID_t docid,
  ...
  if (res == 0) {
    ...
    Verify333(wds != NULL);  // allocate failed
    wds->docIDs = AllocateHashTable(5);
    Verify333(wds->docIDs != NULL);  // allocate failed
    wds->word = word;
    kv.value = wds;
    kv.key = wordkey;
...
LinkedList MIProcessQuery(MemIndex index, char *query[], uint8_t qlen) {
  ...
  res = LookupHashTable(index, wordkey, &kv);
  if (res != 1) {  // can't find the query word
  ...
  while (!HTIteratorPastEnd(itr)) {
    ...
    Verify333(res == 1);  // make sure we have content in kv
    sr->docid = kv.key;
    sr->rank = NumElementsInLinkedList(kv.value);
    PushLinkedList(retlist, sr);
    HTIteratorNext(itr);
  }
  HTIteratorFree(itr);
  ...
  for (i = 1; i < qlen; i++) {
    ...
    for (j = 0; j < ne; j++) {
      ...
      res = LookupHashTable(((WordDocSet*)(kv.value))->docIDs,
                              sr->docid, &sink);
      if (res == 1) {  // is
        sr->rank += NumElementsInLinkedList(sink.value);
        LLIteratorNext(llit);
      } else {  // isn't
