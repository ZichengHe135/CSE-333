...
char *ReadFile(const char *filename, HWSize_t *size) {
  ...
  numread = 0;
  while (left_to_read > 0) {  // file still has contents
    // read one thing
    ...
    if (result == -1) {  // read failed
      ...
      // if it is EAGAIN or EINTR, then it is recoverable, no need for exiting
      continue;
    } else if (result == 0) {  // finish reading, reach end of file
      left_to_read = 0;
    } else {  // normal case
      numread += result;
      left_to_read -= result;
...
HashTable BuildWordHT(char *filename) {
  ...
  if (filecontent == NULL) {  // read file failed
    return NULL;
  }
  if (strlen(filecontent) <= 0) {  // empty
     return NULL;
  }
...
static void LoopAndInsert(HashTable tab, char *content) {
  ...
  while (1) {
    ...
    if (!isalpha(*curptr)) {  // current is not letter
      if (curptr != content && isalpha(*(curptr - 1))) {  // word tail
        *curptr = '\0';
        AddToHashTable(tab, wordstart, (wordstart - content));
      }
    } else {  // is letter
        *curptr = tolower(*curptr);
        if (curptr != content && !isalpha(*(curptr - 1))) {  // word head
           wordstart = curptr;
        }
    }
    curptr++;
...
static void AddToHashTable(HashTable tab, char *word, DocPositionOffset_t pos) {
  ...
  if (retval == 1) {
    ...
    if (wp->positions == NULL) {
      exit(EXIT_FAILURE);
    }
    int result = AppendLinkedList(wp->positions, (void*)((intptr_t) pos));
    if (result != true) {  // allocate failed
      exit(EXIT_FAILURE);
    }
    HTKeyValue newKV;
    newKV.key = hashKey;
    newKV.value = (void*) wp;
    result = InsertHashTable(tab, newKV, &kv);
