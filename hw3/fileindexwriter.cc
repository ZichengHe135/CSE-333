...
HWSize_t WriteIndex(MemIndex mi, DocTable dt, const char *filename) {
  ...
  // MISSING: DONE
  mires = WriteMemIndex(f, mi, filesize + sizeof(IndexFileHeader));
  if (mires == 0) {
    fclose(f);
    unlink(filename);
    return 0;
...
static HWSize_t WriteDocPositionListFn(FILE *f,
  ...
  for (i = 0; i < num_pos_ho; i++) {
    ...
    DocPositionOffset_t *p = reinterpret_cast<DocPositionOffset_t*>(&payload);
...
static HWSize_t WriteWordDocSetFn(FILE *f,
  ...
  // Calculate and return the total amount of data written.
  // MISSING (fix this return value): DONE
  return fsize;
}
...
static HWSize_t WriteBucket(FILE *f,
  ...
  if (chainlen_ho > 0) {
    ...
    for (j = 0; j < chainlen_ho; j++) {
      ...
      element_position_rec pos = {(IndexFileOffset_t)nextelpos};
      pos.toDiskFormat();

      if (fseek(f, offset + fsize, SEEK_SET) != 0) {
        LLIteratorFree(it);
        return 0;
      }
...
static HWSize_t WriteHashTable(FILE *f,
  ...
  for (i = 0; i < ht->num_buckets; i++) {
    ...
    HWSize_t sizeb = 0;
    HWSize_t sizebr = 0;
    sizebr = WriteBucketRecord(f, list, next_bucket_rec_offset,
                               next_bucket_offset);
    if (NumElementsInLinkedList(list) != 0) {
      sizeb = WriteBucket(f, list, next_bucket_offset, fn);
    }
    next_bucket_offset += sizeb;
    next_bucket_rec_offset += sizebr;
  }
