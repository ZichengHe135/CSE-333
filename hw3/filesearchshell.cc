...
int main(int argc, char **argv) {
  ...
  hw3::QueryProcessor qp(idxlist, true);

  while (1) {
    cout << "Enter query: " << endl;
    string query;
    getline(cin, query);
    if (cin.eof() || cin.bad()) {  // end query
      cout << "session ended" << endl;
      break;
    }
    for (auto c : query) {  // to lowercase
      c = tolower(c);
    }
    stringstream ss(query);
    vector<string> v;
    string word;
    while (ss >> word) {  // cut the words into the vector
      v.push_back(word);
    }
    if (v.size() == 0) {  // user did not type anything
      cout << "no query received" << endl;
      continue;
    }

    vector<hw3::QueryProcessor::QueryResult> result = qp.ProcessQuery(v);
    if (result.size() == 0) {  // query found nothing
      cout << "[no results]" << endl;
      continue;
    }
    // if query found something, print
    for (HWSize_t i = 0; i < result.size(); i++) {
      cout << result[i].document_name << " (" << result[i].rank << ")" << endl;
    }
  }
