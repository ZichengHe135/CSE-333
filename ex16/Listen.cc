...
// This is an implementation for Listen.h, all the methods are taken
// from CSE 333 course website under lecture 23 with my modification
#include <arpa/inet.h>
#include <assert.h>
#include <errno.h>
#include <netdb.h>
...
void HandleClient(int c_fd, struct sockaddr *addr, size_t addrlen,
  ...
  while (1) {
    ...
    if (res == 0) {
      ...
      clientbuf[res] = '\0';  // set EOF bit
      int wres = fwrite(clientbuf, sizeof(char), res, stdout);
      if (wres == -1) {
