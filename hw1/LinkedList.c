...
void FreeLinkedList(LinkedList list,
  ...
  while (list->head != NULL) {
    LinkedListNodePtr nodeNow = list->head;
    LinkedListNodePtr nodeNext = list->head->next;
    list->head = nodeNext;
    payload_free_function(nodeNow->payload);
    free(nodeNow);
  }
...
bool PopLinkedList(LinkedList list, LLPayload_t *payload_ptr) {
  ...
  if (list->num_elements == 0 || list->head == NULL) {
    return false;
  }
  ...
  *payload_ptr = oldHead->payload;
  list->num_elements--;
  if (list->num_elements == 0) {  // remove the only element
    list->head = NULL;
    list->tail = NULL;
  } else {  // general case
...
bool SliceLinkedList(LinkedList list, LLPayload_t *payload_ptr) {
  ...
  if (list->num_elements == 0 || list->tail == NULL) {
    return false;
  }
...
bool LLIteratorDelete(LLIter iter,
  ...
  LinkedListNodePtr nodeNow = iter->node;
  payload_free_function(nodeNow->payload);
  if (iter->list->num_elements == 1) {  // remove the only element
    ...
    LinkedListNodePtr nodeNow = iter->node;
    LinkedListNodePtr nodeNext = nodeNow->next;
    LinkedListNodePtr nodePrev = nodeNow->prev;
    if (!LLIteratorHasPrev(iter)) {  // removing head
      iter->list->head = nodeNext;
      iter->node = nodeNext;
      nodeNext->prev = NULL;
    } else if (!LLIteratorHasNext(iter)) {  // removing tail
      iter->list->tail = nodePrev;
      iter->node = nodePrev;
      nodePrev->next = NULL;
